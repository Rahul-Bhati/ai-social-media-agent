...
export async function generateTweets(formData) {
    const session = await auth();

    console.log("Session in server action:", session); // ✅ Ensure tokens exist

    if (!session?.accessToken || !session?.accessSecret) {
        console.error("Missing Twitter credentials in session");
    ...
    try {
        ...
        const res = await axios.post(
            `${apiBaseUrl}/api/genrate-tweet`,
            {
                url: formData.get("url"),
                accessToken: session.accessToken,  // ✅ Send token
                accessSecret: session.accessSecret // ✅ Send secret
            },
            {
                headers: {
        ...
        return { error: "Failed to generate tweets." };
    }
}
